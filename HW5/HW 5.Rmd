...
In a randomization test, we want to consider all of the possible permutations of the data. For a repeated measures experiment, we would go through each row (each subject) and permute the columns (the treatments). For our current problem, in each row there are $k! = 3! = 6$ possible permutations, and there are $n = 3$ rows, so there are $6^{3} = 216$ total permutations to consider. For each of these permuted data sets, we calculate the test statistic, $F^{*}_{p}$. We can then calculate a $P-value$ by determining the number of statistics that are as extreme as the original one.
...
where $F^{*}_{p}$ is the statistic calculated on the $p^{th}$ permuted data set. Now, if there are any constant factors, they can be eliminated. Since the degrees of freedom do not change with permutations, they can be cancelled out.  
...
The second term is a constant, so it can be ignored. Also, in this experiment we have $n_i = n$ for all $i$, so the statistic can be reduced to
