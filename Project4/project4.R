...
grid.arrange(results$plots[[1]], results$plots[[2]], results$plots[[3]],
             results$plots[[4]], results$plots[[5]], results$plots[[6]], ncol = 3, nrow = 2)

grid.arrange(ggplot(obs, aes(x, fill = factor(dist))) + 
...
plots <- list()
plots[[1]] <- ggplot(data.frame(p_hat = c(estimates$p_hat[, 1:k]), dist = rep(1:k, each = m)),
                aes(p_hat, fill = factor(dist))) +
  geom_histogram(binwidth = binwidth[1], alpha = 0.4, position = "identity") +
  geom_vline(data = data.frame(x = p, dist = 1:k), aes(xintercept = x, color = factor(dist)))
plots[[2]] <- ggplot(data.frame(mu_hat = c(estimates$mu_hat[, 1:k]), dist = rep(1:k, each = m)),
       aes(mu_hat, fill = factor(dist))) +
  geom_histogram(binwidth = binwidth[2], alpha = 0.4, position = "identity") +
  geom_vline(data = data.frame(x = mu, dist = 1:k), aes(xintercept = x, color = factor(dist)))
plots[[3]] <- ggplot(data.frame(sigma_hat = c(estimates$sigma_hat[, 1:k]), dist = rep(1:k, each = m)),
       aes(sigma_hat, fill = factor(dist))) +
  geom_histogram(binwidth = binwidth[3], alpha = 0.4, position = "identity") +
  geom_vline(data = data.frame(x = sigma, dist = 1:k), aes(xintercept = x, color = factor(dist)))
